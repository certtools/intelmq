...
# You will have an item 'ip' in your event.
    ...
    def process(self):
        ...
        for mapping in self.parameters.mapping.split(","):
            mkey, mvalue = mapping.split(":")  # convert config to array
            mapping_dict[mkey] = mvalue

        # Invert Dict for defaults values
        rev_mapping_dict = dict(zip(mapping_dict.values(),
          mapping_dict.keys()))
        ...
        if report:
            ...
            for row in raw_report.split('\n'):  # For each line
                ...
                rowcount += 1
                # Get detail from parser parameters, will be nice to have
                # it by source parameters.. Avoid adding if parsed
                if not 'feed.name' in rev_mapping_dict:
                    event.add('feed.name', self.parameters.feed_name,
                      sanitize=True)
                if not 'feed.url' in rev_mapping_dict:
                    event.add('feed.url', self.parameters.feed_url,
                      sanitize=True)
                if not 'classification.type' in rev_mapping_dict:
                    event.add('classification.type',
                      self.parameters.classification_type, sanitize=True)
